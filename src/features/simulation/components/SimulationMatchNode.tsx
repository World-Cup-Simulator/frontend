...
export const SimulationMatchNode = ({ match, resultsMode, teamAName, teamBName }: SimulationMatchNodeProps) => {
  ...
  const winnerProbability = winnerSide === 'A' ? match.teamAWinProbability : winnerSide === 'B' ? match.teamBWinProbability : null;

  return (
    <div className="bg-zinc-800 rounded-lg border border-zinc-700/50 overflow-hidden min-w-[150px]">
      {/* Team A */}
      <div className={rowClass('A')}>
        <FlagImage code={match.teamA} alt={teamAName || match.teamA} className="h-3.5 w-5 shrink-0" />
        <span className="text-xs font-medium truncate flex-1 text-left">
          {teamAName || match.teamA}
        </span>
        {resultsMode === 'no-results' && winnerSide === 'A' && winnerProbability !== null && (
      ...
      <div className={rowClass('B')}>
        <FlagImage code={match.teamB} alt={teamBName || match.teamB} className="h-3.5 w-5 shrink-0" />
        <span className="text-xs font-medium truncate flex-1 text-left">
          {teamBName || match.teamB}
        </span>
        {resultsMode === 'no-results' && winnerSide === 'B' && winnerProbability !== null && (
          <span className="text-[10px] font-bold text-emerald-400 shrink-0">
            {winnerProbability}%
          </span>
        )}
